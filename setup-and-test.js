  ...
  try {
    ...
    return new Promise((resolve) => {
      exec('npm install node-fetch@2', (error) => {
        if (error) {
          console.error('❌ Error instalando node-fetch:', error.message)
          resolve(false)
        } else {
...
const setupAndTest = async () => {
  ...
  if (!hasFetch) {
    console.error('❌ No se pudo instalar node-fetch. Instálalo manualmente: npm install node-fetch@2')
    return
  }
  ...
  if (!isRunning) {
    ...
    console.log('   Docker: docker run -ti --rm -p 5000:5000 libretranslate/libretranslate')
  ...
  if (testResult) {
    ...
    console.log('📝 Ahora puedes habilitar la traducción automática en tu script:')
    ...
    console.log('🔧 Verifica que LibreTranslate esté funcionando correctamente.')
  }
}
...
const autoConfigureScript = async () => {
  ...
  if (answers.enableAutoTranslation) {
    const fs = require('fs')
    const scriptPath = './translation-script-improved.js'
    ...
    try {
      ...
      console.log('✅ Script principal configurado para usar traducción automática!')
    } catch (error) {
